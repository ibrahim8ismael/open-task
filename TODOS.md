...
- [ ] B0.5 Auth endpoints `docs/04 §4.1`: `GET /auth/get-csrf-token/` ✅, `POST /auth/email-check` (stub) ✅; TODO: `sign-in|sign-up|sign-out` (bcrypt + session cookie), magic, oauth `google|github`, password flows, rate-limit
- [ ] B0.6 Workspace endpoints `docs/04 §4.2` + Project base `docs/04 §4.3`: stubs return `{results:[],count:0}` ✅; TODO: Prisma-backed CRUD, slug-check, invitations+join, members+leave, last-visited, user-properties, default-state seed on project create
